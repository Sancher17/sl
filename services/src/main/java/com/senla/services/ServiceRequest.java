...
public class ServiceRequest extends Service implements IServiceRequest {
    ...
    public ServiceRequest(IRequestDao requestDao, IFileWorker fileWorker) {
    ...
    public void exportToCsv() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()){
            super.writeToCsv(requestDao.getAll(session, Request.class), fileWorker);
        } catch (HibernateException e) {
    ...
    public void importFromCsv() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            List<Request> importListFromFile = fileWorker.importListFromFile(PATH_REQUEST_CSV, session, Request.class, requestDao);
