...
import static com.senla.constants.Constants.PATH_ORDER_CSV;
...
public class ServiceOrder extends Service implements IServiceOrder {
    ...
    public ServiceOrder(IOrderDao orderDao, IBookDao bookDao, IFileWorker fileWorker) {
    ...
    public void exportToCsv() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()){
            super.writeToCsv(orderDao.getAll(session, Order.class), fileWorker);
        } catch (Exception e) {
    ...
    public void importFromCsv() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            List<Order> importListFromFile = fileWorker.importListFromFile(PATH_ORDER_CSV, session, Order.class, orderDao);
